...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("auto_replay_enabled", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column("timeout_connect_ms", sa.Integer(), nullable=False, server_default=sa.text("3000")),
        sa.Column("timeout_read_ms", sa.Integer(), nullable=False, server_default=sa.text("10000")),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("now()"),
    ...
    op.create_table(
        ...
        sa.Column("published_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("publish_attempts", sa.SmallInteger(), nullable=False, server_default=sa.text("0")),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
...
def downgrade() -> None:
    ...
    op.drop_index("ix_partner_endpoints_event_types", table_name="partner_endpoints", postgresql_using="gin")
