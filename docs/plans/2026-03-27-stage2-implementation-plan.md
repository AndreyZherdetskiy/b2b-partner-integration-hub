...
`payload` is the Kafka JSON envelope (UUID public ids). `key` is partner `public_id` string stored inside payload or a `message_key` field — add `message_key VARCHAR(64)` column if not present (Alembic revision). If adding a column, new revision `20260316_0003_outbox_message_key.py`.
...
**Files:**
- Create: `app/domain/models/signing_secret.py`, `alembic/versions/20260316_0004_partner_signing_secrets.py`, `app/domain/services/signing_secrets.py`, `tests/unit/test_signing_secret_rotation.py`
